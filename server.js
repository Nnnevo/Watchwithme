...
server.listen(3000, '127.1.1.131', function() {
  console.log('Server running on http://127.1.1.131:3000');
});
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on('disconnect', function(data) {
        ...
        // Якщо ви не останній сокет, який виходить, і у кімнаті є підключені сокети
        if (room && room.sockets) {
            const socketIds = Object.keys(room.sockets);
            if (socketIds.length > 0) {
        // Перепризначення обов'язків хоста першому сокету у списку
            console.log("привіт, я хост " + socket.id + " і я залишаю свої обов'язки " + socketIds[0]);
            io.to(socketIds[0]).emit('autoHost', { roomnum: roomnum });
        }
...
if (!io.sockets.adapter.rooms['room-' + socket.roomnum]) {
    io.sockets.adapter.rooms['room-' + socket.roomnum] = {
        ...
        // інші властивості кімнати, якщо потрібно
    };
} else {
    // Sets the host if the room already exists
    io.sockets.adapter.rooms['room-' + socket.roomnum].host = host
}
